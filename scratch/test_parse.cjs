...
const products = parseWhatsAppCatalog(rawText);

function filterByGpu(mode) {
  return products.filter(p => {
    if (mode === '2gb') {
      return p.isDedicatedGpu && (p.gpuVram === 2 || /\b2\s*gb\b/i.test(p.gpu));
    }
    if (mode === '4gb') {
      return p.isDedicatedGpu && (p.gpuVram >= 4 || /\b4\s*gb\b/i.test(p.gpu));
    }
    if (mode === '12gb') {
      return p.isDedicatedGpu && (p.gpuVram >= 12 || /\b12\s*gb\b/i.test(p.gpu));
    }
    if (mode === 'dedicated') {
      return p.isDedicatedGpu;
    }
    return true;
  });
}

console.log("--- 2GB GPU FILTER RESULTS (" + filterByGpu('2gb').length + ") ---");
filterByGpu('2gb').forEach(p => console.log("  -> " + p.title + " | " + p.gpu + " (" + p.gpuVram + "GB)"));

console.log("\n--- 4GB GPU FILTER RESULTS (" + filterByGpu('4gb').length + ") ---");
filterByGpu('4gb').forEach(p => console.log("  -> " + p.title + " | " + p.gpu + " (" + p.gpuVram + "GB)"));

console.log("\n--- 12GB GPU FILTER RESULTS (" + filterByGpu('12gb').length + ") ---");
filterByGpu('12gb').forEach(p => console.log("  -> " + p.title + " | " + p.gpu + " (" + p.gpuVram + "GB)"));
